...
class SiteOrigin_Widget_Image_Widget extends SiteOrigin_Widget {
		...
		return array(
			...
			'image_shape' => array(
				...
				'fields' => array(
					'enable' => array(
						'type' => 'checkbox',
						'label' => __( 'Enable', 'so-widgets-bundle' ),
						'default' => false,
						'state_emitter' => array(
							'callback' => 'conditional',
							'args' => array(
								'image_shape[enabled]: val',
								'image_shape[disabled]: ! val',
							),
						),
					),
					'shape' => array(
					...
					'repeat' => array(
						'type' => 'select',
						'label' => __( 'Repeat', 'so-widgets-bundle' ),
						'options' => array(
							'no-repeat' => __( 'No Repeat', 'so-widgets-bundle' ),
							'repeat' => __( 'Repeat', 'so-widgets-bundle' ),
							'space' => __( 'Repeat (No Clipping)', 'so-widgets-bundle' ),
							'repeat-x' => __( 'Repeat Horizontally', 'so-widgets-bundle' ),
							'repeat-y' => __( 'Repeat Vertically', 'so-widgets-bundle' ),
						),
						'state_handler' => array(
							'image_shape[enabled]' => array( 'show' ),
							'image_shape[disabled]' => array( 'hide' ),
	...
	public function get_template_variables( $instance, $args ) {
		...
		$attr = siteorigin_loading_optimization_attributes(
			$attr,
			'image',
			$instance,
			$this
	...
	public function get_less_variables( $instance ) {
		...
		if ( ! empty( $instance['image_shape']['enable'] ) ) {
			// Instances saved before the shape field defaulted an empty
			// value store '' here. Treat that as the circle the picker
			// previews, so those widgets render without a re-save.
			$shape = isset( $instance['image_shape']['shape'] ) && $instance['image_shape']['shape'] !== '' ? $instance['image_shape']['shape'] : 'circle';

			if ( SiteOrigin_Widget_Image_Shapes::single()->is_valid_shape( $shape ) ) {
				...
				$less_variables['image_shape'] = 'url( "' . esc_url( SiteOrigin_Widget_Image_Shapes::single()->get_image_shape( $shape ) ) . '" )';
			}
		}
